...
def showmessages():
    ...
    channel_id = request.args.get('channel_id') #must be valid channel
    start = request.args.get('start') #cannot be >= no. of messages in channel
    end = send + 50
...
def send():
    ...
    if(u_id not in newchannel['members'] or u_id not in newchannel['owners']):
    ...
    return send_success({'message_id': message_id})
